...
public final class SpeechTimelineView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (SessionSpeechInterval interval : intervals) {
            float left = (interval.getStartOffsetMillis() / (float) durationMillis) * width;
            float right = (interval.getEndOffsetMillis() / (float) durationMillis) * width;
            rect.set(left, 0f, Math.max(left + 2f, right), height);
            canvas.drawRoundRect(rect, radius, radius, speechPaint);
        }
    }
}
